      ...
      <div class="container">
        <div class="row mb50">
          ...
          <div class="sec-sub-title text-center wow rubberBand animated" data-wow-duration="1000ms">
            <p> TO SEE DETAILS OF THE CANDIATE TO ALLOT BRANCH</p>
          </div>
...
<?php }?>
			...
			<div class="container">
				<div class="row mb50">
					...
					<div class="sec-sub-title text-center wow rubberBand animated" data-wow-duration="1000ms">
						<p> SELECT THE DEPARTMENT TO BE ALLOTTED</p>
					</div>
					<div class="col-md-offset-3 col-md-10 list">
					<br>
...
$con=mysql_connect("localhost","root","")or die("canot connect");
		...
		if(isset($_POST['btn1']))
			{ 
  					...
  					if($d_br=='mech'){
  				echo "<script>alert('Mech');</script>";	
 				$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='mech' ";}
  				elseif ($d_br=='cs') {
  					echo "<script>alert('CS');</script>";
  					$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='cs' ";
  				}
  				elseif ($d_br=='civil'){ 
  					echo "<script>alert('Civil');</script>";
  					$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='civil' ";}
  				elseif ($d_br=='eee') {
  					echo "<script>alert('EEE');</script>";
  					$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='eee' ";
  				}
  				elseif ($d_br=='ec') {
  					echo "<script>alert('EC');</script>";
  					$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='ec' ";
  				}
  				elseif ($d_br=='aie') {
  					echo "<script>alert('AIE');</script>";
  					$upQuery= "update  seat set seat_allot=seat_allot+1 where branch='aie' ";
  									}
  				
 					$insQuery= "insert into students_details(name,appno,branch) values('{$d_name}',{$d_no},'{$d_br}')";
					...
					mysql_query($upQuery,$con) or die(mysql_error());
					//echo $insQuery;
